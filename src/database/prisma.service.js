...
var PrismaService = /** @class */ (function (_super) {
    ...
    function PrismaService() {
        ...
        _this.logger = new common_1.Logger(PrismaService_1.name); // Typing fix
        _this.softDeleteModels = [
            'users'
        ];
        _this.$on('query', function (e) {
            _this.logger.log('Query: ' + e.query);
            _this.logger.log('Params: ' + e.params);
            _this.logger.log('Duration: ' + e.duration + 'ms');
        });
    ...
    PrismaService.prototype.enableShutdownHooks = function (app) {
        this.$on('beforeExit', function () { return app.close(); });
    };
    PrismaService.prototype.format = function (data) {
        return JSON.parse(JSON.stringify(data, function (_, value) {
            return typeof value === 'bigint' ? parseInt(value.toString(), 10) : value;
        }));
    };
    var PrismaService_1;
